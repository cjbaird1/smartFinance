...
const EducationCenter = () => {
  ...
  useEffect(() => {
    ...
    educationTopics.forEach((mainTopic, mainIndex) => {
      if (mainTopic.subSections) {
        const subIndex = mainTopic.subSections.findIndex(
          (subTopic) => toSlug(subTopic.title) === hash
        );
        if (subIndex !== -1) {
          foundMainIndex = mainIndex;
          foundSubIndex = subIndex;
        }
      }
    });

    if (foundMainIndex !== null && foundSubIndex !== null) {
      setOpenMainSection(foundMainIndex);
      setOpenSubSection(foundSubIndex);
      setTimeout(() => {
        const ref = sectionRefs.current[hash];
        if (ref) {
  ...
  const toggleMainSection = (index) => {
    setOpenMainSection(openMainSection === index ? null : index);
      ...
      <div className="education-sections">
        {educationTopics.map((topic, index) => (
          <div key={index} className="education-section">
            <button
              className={`section-header ${openMainSection === index ? 'open' : ''}`}
              onClick={() => toggleMainSection(index)}
            >
              <span>{topic.title}</span>
              <span className="arrow">{openMainSection === index ? '▼' : '▶'}</span>
            </button>
            {openMainSection === index && (
                  ...
                  <div className="sub-sections">
                    {topic.subSections.map((subSection, subIndex) => (
                      <div
                        ...
                        ref={(el) => (sectionRefs.current[toSlug(subSection.title)] = el)}
                      >
                        <button
                          className={`sub-section-header ${openSubSection === subIndex && openMainSection === index ? 'open' : ''}`}
                          onClick={() => toggleSubSection(subIndex)}
                        >
                          <span>{subSection.title}</span>
                          <span className="arrow">{openSubSection === subIndex && openMainSection === index ? '▼' : '▶'}</span>
                        </button>
                        {openSubSection === subIndex && openMainSection === index && (
